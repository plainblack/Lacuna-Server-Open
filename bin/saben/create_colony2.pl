...
my $body = $bodies->search(
    { x => {between => [ $target_home->x - 10, $target_home->x + 10 ]}, y => { between => [ $target_home->y - 10, $target_home->y + 10 ]  }, empire_id => undef, size => { between => [30,35]}},
    { rows => 1 }
    )->single;
...
my @plans = (
    ['Lacuna::DB::Result::Building::Permanent::Ravine',$half_level, 3, -3],
    ['Lacuna::DB::Result::Building::Waste::Sequestration',10, 3, -4],
    ...
    ['Lacuna::DB::Result::Building::PilotTraining',$half_level, 3, 3],
    ['Lacuna::DB::Result::Building::Propulsion',$half_level, 3, 2],
    ['Lacuna::DB::Result::Building::Trade', $max_level, 4, 2],
