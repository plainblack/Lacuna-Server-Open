...
while (my $star = $stars_rs->next) {
    ...
    # Get the hash of stations to alliance ID
    my $station_id = $star->station_id;
    if ($station_id) {
        my $alliance = $star->station->alliance;
        if (not $alliance_ref->{$alliance->id}) {
            my ($r, $g, $b);
        ...
        $clr = $alliance_color->{$alliance->id};
        $station_to_ally->{$station_id} = $alliance->id;
    }
    draw_star($x, $y, $clr);
}
