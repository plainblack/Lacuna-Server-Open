...
B<NOTE:> You should probably B<never> call this method directly, as it is a wasted call 
since the data it returns comes back in the status block of every relevant request. 
See L<Intro/"Status"> for details.
...
Retrieves a list of the buildings on a planet. The surface of all planet's 
...
The C<get_body> method (among others) will give you C<size> and C<building_count>. 
...
=head2 get_buildable_locations ( options )
...
A hash of options.  Currently, the only valid key is C<notes>, and its
