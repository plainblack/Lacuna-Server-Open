=head1 Theme Park Methods

Theme Park is accessible via the URL C</themepark>.
...
This method is changed from the default because it adds a C<themepark> element to the output.
...
Consult the C<work> block to see how long the Theme Park will remain in operation.
...
=head2 operate ( session_id, building_id )
