...
Returns 1 if the name is valid and available.
...
Your secret password (don't share your empire password with anyone!)
...
=head2 create

  {
    ...
    "invite_code"           :"",
    "captcha_challenge"     :"03AH...OOsY",
    "captcha_response"      :"raidnB work",
    "species_name"          :"Average",
    "species_description"   :"Not specializing in any area, but without any particular weaknesses.",
    "species_min_orbit"     :3,
    "species_max_orbit"     :3,
...
If an error is thrown then a new reCaptcha will need to be generated. A reCaptcha cannot be used more than once.
...
The password to log in to the empire. Must be between 6 and 30 characters. Required unless you have a valid C<facebook_uid> and C<facebook_token>. Still required even if you are authenticating using Facebook.
...
=head3 species_max_orbit (required)
...
=head3 current_password (required)

The current empire password.
...
=head2 boost_food ( session_id [, weeks] )
...
=head2 boost_water ( session_id [, weeks] )
...
=head2 boost_energy ( session_id [, weeks] )
...
=head2 boost_ore ( session_id [, weeks] )
...
Spend 5 essentia and boost the specified type on all planets for and additional 7 days.
...
=head2 boost_happiness ( session_id [, weeks] )
...
Should be one of 'storage', 'food', 'water', 'energy', 'ore', 'happiness', 'building', 'ship_build', 'ship_speed'

=head3 RESPONSE.

It responds with the same as C<get_boosts>


=head2 boost_building ( session_id [, weeks] )

=head2 enable_self_destruct
...
=head2 boost_spy_training ( session_id [, weeks] )
