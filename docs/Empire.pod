...
A 36 character code that was sent to the user by a friend. It is usable only once, and will ensure that their friend gets a home planet that is in relatively close proximity to their home planet. Can be passed in here, or in C<found>, whichever works best for the UI.
...
A 36 character code that was sent to the user by a friend. It is usable only once, and will ensure that their friend gets a home planet that is in relatively close proximity to their home planet. Can be passed in here, or in C<create>, whichever works best for the UI.
...
Updates the empire's species and returns 1. Can only be called after C<create> has been called and before C<found> has been called. Before or after that will throw an exception.
