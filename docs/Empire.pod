...
B<NOTE:> You should probably B<never> call this method directly, as it is a wasted call since the data it returns comes back in the status block of every relevant request. See L<Intro/"Status"> for details.
...

 {
    ...
    "empire" : {
        ...
        "space_stations" : {
            "id-goes-here" : "SS Alpha",
            "id-goes-here" : "SS Gamma"
        },
...
NOTE: Space Stations are no longer returned in 'planets' but are returned in a separate
section called 'space_stations'
...
=head2 get_planets ( session_id )

Returns information about the planets held by this empire.
...
=head2 spy_training_boost ( session_id )
