...
=head2 boost_rpc ( session_id )

Spends 25 essentia, and boosts the number of RPC calls allowed per minute by 50% as well as doubling the total number RPC calls per day. If a boost is already underway, calling again will add 7 more days.

NOTE: The boost will not affect the 'rpc_limit' returned as a part of server status.
