...
=head2 get_resource_ships ( session_id, building_id )

Returns a list of the ships that are either working to transport resources or available.
...
=head2 view_resource_chains ( session_id, building_id )

Returns a list of the resource chains currently controlled by this Trade Ministry
...
You can set up three resource-chains for each level of your Trade Ministry. Each 
resource-chain can transfer any amount of any one resource (food type, ore type
energy, waste or water).
...
=head2 create_resource_chain (session_id, building_id, resource_type, resource_hour)

Create a new resource chain.
...
Returns the same as the call to B<view_resource_chains>


=head2 delete_resource_chain (session_id, building_id, resource_chain_id)

Delete the resource chain identified by the B<resource_chain_id>

Returns the same as the call to B<view_resource_chains>


=head2 update_resource_chain ( session_id, building_id, waste_chain_id, resource_type, resource_hour)

Change the B<resource_type> and B<resource_hour> for the Resource Chain specified by B<resource_chain_id>
...
B<resource_hour> if you don't have enough ships to transfer the amount of resorces in all
resource chains.

You may set the resource_hour to zero if you want to suspend the resource chain to the target planet.

If you want to remove the resource chain totally then call B<delete_resource_chain>
...
=head2 add_resource_ship_to_fleet (session_id, building_id, ship_id )

Take a ship from your space port and add it to the ships servicing the resource chains.
...
=head2 remove_resource_ship_from_fleet (session_id, building_id, ship_id )

Remove a ship from servicing your resource chains and tell it to return to the space port.
