...
=head2 view_taxes_collected (session_id, building_id )
...
Take control of a star. Usable at parliament level 7.
...
=head2 propose_taxation ( session_id, building_id, taxes )
...
=head2 propose_foreign_aid ( session_id, building_id, planet_id, resources )
...
=head2 propose_members_only_colonization ( session_id, building_id )
...
=head2 propose_members_only_excavation ( session_id, building_id )
...
=head2 propose_neutralize_bhg ( session_id, building_id )

If passed, Black Hole Generators will not be able to be used on or from systems in the jurisdiction of this station.
