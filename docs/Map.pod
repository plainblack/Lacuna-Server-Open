...
This just returns enough data to draw the starmap.
...
return most of the data for planets you occupy (ore, water etc.) 
You can make separate calls to the /body get_status for that information.

The B<alliance> section is only returned if the star is influenced by an Alliance.

B<seized> indicates if the star is seized by the B<alliance> which has the most influence.
Note that a star may be under the influence of an alliance, but if it is less than 50 it
will not be enough to seize it.

The B<empire> section is only returned if the body is occupied by an empire.

=head2 get_stars ( session_id, x1, y1, x2, y2 )

Note: use of this API is no longer supported.
...
=head2 search_stars (session_id, name, alliance_id)
...
(optional) if provided only stars seized by that alliance will be returned.
