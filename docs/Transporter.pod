...
An integer which represents how much essentia you are asking for in this trade. Must be between 1 and 99.
...
=head2 accept_from_market ( session_id, building_id, trade_id, captcha_guid, captcha_solution )
...
The C<view_market> list also includes a captcha guid. That string must be passed along with the trade. The guid used must be the most recent one returned (if you're caching the list of trades in some UI).

=head3 captcha_solution

The captcha described by C<view_market> will require the user to solve a simple puzzle to prove their humanity. The answer to that puzzle must be provided here.
