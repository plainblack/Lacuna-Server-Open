...
An array reference containing an ordered list of up to four glyph ids or up to four glyph types.

=head3 quantity

An optional quantity; if left blank it defaults to 1, otherwise specify the number of times to assemble the indicated glyphs up to a maximum of 50.  If greater than one, glyphs must specify types, not ids.
