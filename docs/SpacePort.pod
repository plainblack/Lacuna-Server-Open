...
Travelling fleets will have a B<from> and B<to> block and a B<date_arrives> value. 

Defending fighters will have a B<from> and B<orbiting> block.
...
Recall all, or part of a fleet that have the task of 'Defend'
or 'Orbiting' back to their body of origin.
