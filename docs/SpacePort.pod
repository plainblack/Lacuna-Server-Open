...
=head2 view_all_fleets ( session_id, building_id, [ paging, filter, sort ] )

Shows you all your fleets whatever they are doing.
...
Travelling fleets will have a from and to block. Defending fighters will have a from and orbiting block.
...
=head2 view_incoming_fleets ( session_id, building_id, [ paging, filter, sort ] )

Shows you all fleets not based on this colony which are incoming.

However the list is filtered depending upon the stealth of the ship vs the level of the spaceport.

To see the C<from> block :-

 SpacePort Level * 450 >= Fleet Stealth

To see the ship type and name :-

 SpacePort level * 350 >= Fleet Stealth

Note that you will always see full info for your own, or allied incoming ships.

 {
   "fleets" : [
     ...
     },
     {
       ...
       "type" : "Unknown",
       "type_human" : "Unknown",
       "date_arrives" : "02 01 2010 14:33:01 +0600",
       "from" : {},
     }
     ...
   ],
...
The fleets are sorted to show the earliest to arrive at the top of the list.
ordered with the ones due to arrive first at the top of the list
...
=head2 get_available_for

=head2 get_unavailable_for


=head2 get_incoming_for ( session_id, target, [ paging, filter, sort ] )

Shows a list of all fleets incoming to the target, either from your own empire or from your allies.

If the target is a colony or Space Station of yours or one of your allies, then it will also show
all foreign incoming fleets.
...
B<association> in the B<from> section specifies either 'Own', 'Allied" or "Foreign" depending upon
...
The target to send the fleet to. See above.
...
=head2 get_orbiting_for ( session_id, target, [ paging, filter, sort ] )

Show a list of all fleets orbiting the target, either from your own empire or from your allies.
...
The target to check for orbiting fleets. See above.
...
=head2 get_mining_for ( session_id, target )
...
The target to check for mining platforms. See above.
...
=head2 view_fleets_travelling ( session_id, building_id, [ page_number ])

Returns a list of the fleets that are travelling to or from this planet.

B<NOTE:> All inbound/outbound fleetss are shown regardless of which space port they will eventually land at.
...
The total B<number_of_fleets_travelling> and the total B<number_of_ships_travelling> in those fleets are returned.

The array reference B<fleets_travelling> shows the number of fleets, and the B<quantity> of ships of each B<type> in each fleet.
...
Defaults to 1. An integer representing which page of fleets travelling to view. Each page shows 25 fleets.
