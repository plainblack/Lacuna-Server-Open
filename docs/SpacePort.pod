...
The C<unavailable> block tells you about ships that are docked, but are for some reason not available to be used. The C<mining_platforms> block will be returned only if the target is an asteroid that has platforms on it.
...
=head2 recall_ship ( session_id, ship_id )
