...
=head2 view_all_ships ( session_id, building_id, [ page_number, items_per_page, filter, sort ] )
...
Optional. A hash reference that contains the filter criteria. Valid filter option keys include task, type and tag. Valid tasks include Docked, Building, Mining, Travelling, and Defend. Valid types are the existing ship types. Valid tags include Trade, Colonization, Intelligence, Exploration, War, and Mining. The filter option values should be passed as an array reference or scalar. Tags will be converted to types prior to filtering. (This means that if tags and types are passed together, the types will be overwritten when the tags are converted.)
...
Shows you all the foreign ships that are incoming. However, the list is filtered by the stealth of the ship vs the level of the SpacePort. The formula is:
 ...
 300 * SpacePort Level >= Ship Stealth
...
The C<from> block is only included if
 ...
 100 * SpacePort Level >= Ship Stealth
...
The C<unavailable> block tells you about ships that are docked, but are for some reason not available to be used. The C<recallable> block will be returned only if the target is body and tells you about fighters that are defending and can be recalled. The C<mining_platforms> block will be returned only if the target is an asteroid that has platforms on it.
...
=head2 send_fleet ( session_id, ship_ids, target )
...
Recalls a ship from allied defense ('Defend' task) to the body of origin.
