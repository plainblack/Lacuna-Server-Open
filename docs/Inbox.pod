...
=head2 view_inbox ( session_id, [ page_number ] )

Displays a list of the messages in the empire's inbox.
...
Archives a list of messages. Returns C<view_inbox>.

B<NOTE:> You cannot archive sent messages.
...
A comma separated string of empire names.

=head3 subject

A subject for the message. It cannot contain any special characters, and must be less than 100 characters in length.

=head3 body

The body of the message. It cannot contain any special characters, and must be less than 200,000 characters in length.
