...
sub propose_seize_star {
    ...
    if ($self->in_neutral_zone) {
        confess [1009, 'Your station is in the Neutral Zone and is not allowed to seize any star.'];
    }
    my $star = Lacuna->db->resultset('Lacuna::DB::Result::Map::Star')->find($star_id);
    unless (defined $star) {
        confess [1002, 'Could not find the star.'];
    }
    if ($star->in_neutral_zone) {
        confess [1009, 'That star is in the Neutral Zone and can not be seized.'];
    }
    if ($star->station_id) {
