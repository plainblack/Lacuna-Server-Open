...
sub www_view_essentia_log {
    ...
    $out .= '<table style="width: 100%;"><tr><th>Date</th><th>Amount</th><th>Description</th><th>Transaction ID</th></tr>';
    while (my $transaction = $transactions->next) {
        $out .= sprintf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>', $transaction->date_stamp, $transaction->amount, $transaction->description, $transaction->transaction_id);
    }
    $out .= '</table>';
    return $self->wrap($out);
}
...
sub www_view_ships {
    ...
    while (my $ship = $ships->next) {
        ...
        if ($ship->task eq 'Travelling') {
            ...
            $out .= sprintf('<td>%s<br>%s<form method="post" action="/admin/recall/ship"><input type="hidden" name="ship_id" value="%s"><input type="hidden" name="body_id" value="%s"><input type="submit" value="recall"></form></td>', $ship->task, $target->name, $ship->id, $body_id);
		}
