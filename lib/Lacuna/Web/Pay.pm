...
sub itransact_buy_url {
    my ($self, $user_id) = @_;
    return '/buy/currency/?user_id='.$user_id;
}
...
sub www_buy_currency_cc {
    ...
    %payload = (
        GatewayInterface    => {
            ...
            AuthTransaction => {
                ...
                Description         => 'Essentia Order #'.$request->param('order_number'),
                AccountInfo         => {
                    CardAccount => {
    ...
    if ($response->is_success) {
        ...
        if($result->{Status} eq 'ok') {
            ...
            #return $self->itransact_rejected($result);
            
        }
    }
    else {
        confess [1009, 'Could not connect to the credit card processor.'];
        #return $self->itransact_error($result);
    }
}


sub itransact_rejected {
    my ($self, $result) = @_;
    return $self->format_error('Shucks! Your card was rejected: '.$result->{XID});
}
    ...
    my $content = <<EoHTML;
        <script type="text/javascript">
            function update_currency(el) {
                ...
                var currency = Dom.get("premium_currency_amount");
                currency.value = value[0];
            }
        </script>
        <h2>Get More Essentia</h2>
        <form action="/buy/currency/cc" id="cc_form">
            <div>
                <label for="total">Buy</label>
                    <input type="hidden" name="user_id" id="user_id" value="$user_id">
                    <input type="hidden" name="premium_currency_amount" id="premium_currency_amount" value="600">
                ...
                <label for="pay_by_cc">Pay by credit card</label><img src="/some/path/to/cc/image.png">
                <label for="card_number">Card Number:</label><input name="card_number" id="card_number" type="text" size="18" autocomplete="off">
                ...
                <label for="cvv2">csc:</label><input name="cvv2" id="cvv2" type="text" size="3">
