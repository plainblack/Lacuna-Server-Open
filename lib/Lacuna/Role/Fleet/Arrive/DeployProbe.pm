package Lacuna::Role::Fleet::Arrive::DeployProbe;
...
after can_send_to_target => sub {
    ...
    my $count = Lacuna->db->resultset('Probes')->search({ body_id => $body->id })->count;
    $count += Lacuna->db->resultset('Fleet')->search({ body_id => $body->id, type=>'probe', task=>'Travelling' })->count;
    my $max_probes = 0;
