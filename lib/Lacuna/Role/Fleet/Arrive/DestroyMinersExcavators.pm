package Lacuna::Role::Fleet::Arrive::DestroyMinersExcavators;
...
#   not an asteroid or uninhabited planet
    ...
    if ($is_asteroid) {
        my $platforms = Lacuna->db->resultset('MiningPlatforms')->search({asteroid_id => $self->foreign_body_id });
    ...
    # Destroy Excavs
    my $excavs = Lacuna->db->resultset('Excavators')->search({body_id => $self->foreign_body_id });
