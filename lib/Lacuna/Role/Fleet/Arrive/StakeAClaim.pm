package Lacuna::Role::Fleet::Arrive::StakeAClaim;
...
after can_send_to_target => sub {
    my ($self, $target) = @_;
    my $empire = $self->body->empire;
    if ($target->isa('Lacuna::DB::Result::Map::Body::Planet::GasGiant') &&
        $empire->university_level < 19) {

        confess [1009, 'Can only be sent to habitable planets.'];
    }
    if ($empire->university_level < 18 &&
        ($target->orbit > $empire->max_orbit || $target->orbit < $empire->min_orbit)) {
        confess [1009, 'Your species cannot survive on that planet.' ];
    }
    my $stake = Lacuna->cache->get('stake', $self->body->empire->id);
    if (defined $stake && $stake >= 3) {
        confess [1010, 'You have already sent 3 stakes in a short period of time. Wait 24 hours since the last stake and send again.'];
    }
    return 1;
};
