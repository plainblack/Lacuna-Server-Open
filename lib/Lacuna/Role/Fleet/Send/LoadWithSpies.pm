package Lacuna::Role::Fleet::Send::LoadWithSpies;
...
after send => sub {
    ...
    return if ( $self->direction eq 'in' && $self->type ne 'spy_shuttle' ); # load outbound fleets and inbound spy shuttles
...
after can_send_to_target => sub {
    my ($self, $target) = @_;
    return if ( $self->direction eq 'in' ); # Only applies to outbound fleets
