...
use constant prereq         => { class=> 'Lacuna::DB::Result::Building::DemolitionLab',  level => 10 };
...
sub arrive {
    ...
    unless ($self->trigger_defense) {
        $self->damage_building($self->foreign_body->get_building_of_class('Lacuna::DB::Result::Building::Observatory'));
    }
}
