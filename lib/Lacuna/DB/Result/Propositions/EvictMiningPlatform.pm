package Lacuna::DB::Result::Propositions::EvictMiningPlatform;
...
before pass => sub {
    my ($self) = @_;
    my $platform = Lacuna->db->resultset('Lacuna::DB::Result::MiningPlatforms')->find($self->scratch->{platform_id});   
