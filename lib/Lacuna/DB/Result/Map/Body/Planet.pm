...
sub spend_food {
    ...
    if ($food_type_count > 3) {
        ...
        my $empire = $self->empire;
        if (!$empire->skip_resource_warnings && !$empire->check_for_repeat_message('complaint_food_diversity')) {
...
sub add_waste {
    ...
    if ($store < $storage) {
        ...
        if (!$empire->skip_pollution_warnings && !$empire->check_for_repeat_message('complaint_pollution')) {
            $empire->send_predefined_message(
